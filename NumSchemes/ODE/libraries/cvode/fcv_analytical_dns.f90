! CVODE fortran example from the documentation
! Solves a simple example problem using Newton iteration on a dense matrix
...
contains
    ...
    integer(c_int) function RhsFn(tn, sunvec_y, sunvec_f, user_data ) result(ierr) bind(C, name="RhsFn")
        ...
        ! Get data from sundials vectors
...
program main
    ...
    real(c_double) :: rtol, atol    ! tolerances
